...
2. **Flujo de cobro (Mercado Pago, EN PRODUCCIÓN)**: `/upgrade` → `/api/checkout` (Preference, precio/créditos del catálogo server-side, nunca del cliente) → webhook acredita **idempotente** (`process_mp_payment`). Lo más sensible a testear: que un mismo `payment_id` NO acredite dos veces, y que la firma `x-signature` inválida se rechace.
