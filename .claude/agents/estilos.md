...
⚠️ **Corrés como subagente: el hook SessionStart NO te inyecta nada.** Para el estado general, hacé `Read` por ruta absoluta de los DOS archivos de memoria (los ÚNICOS; `VENDI_DOC.md`/`MEMORY.md` están MUERTOS): `C:\Users\Usuario\vendiapp\vendi\cerebro_vendi\MEMORIA_DE_DIOS.md` (proyecto) + `C:\Users\Usuario\vendiapp\vendi\cerebro_vendi\MINIONS.md` (agentes).
